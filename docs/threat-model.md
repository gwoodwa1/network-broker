...
| Prompt injection, structural manipulation or poisoned device text | Preserve hostile captured bytes separately; apply versioned bounded sanitisation, Unicode-normalized detection, safe quarantine, manifest- and media-type-bound strict schema promotion, field-specific identifier grammar and signed field-level taint | Novel obfuscation, unsupported future recipe schemas or an upstream consumer that treats tainted data as instructions |
...
| Disclosure receipt tampering or signature stripping | Bind tenant, actor, evidence, policy lineage and delivered-payload digest into a domain-separated signature set; enforce minimum and required-algorithm verification policy; persist receipts append-only | External transparency anchoring remains future work |
| Cross-tenant read or workflow-activity inference | Include tenant in every durable lookup and make cross-tenant absence indistinguishable from missing; resolution status/watch derive tenant from verified SPIFFE identity; watch exposes per-resolution versions rather than global outbox sequences | Query introduced without tenant predicate; status/watch timing and connection behavior still require deployment measurement |
