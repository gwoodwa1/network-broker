...
| Cross-tenant read | Include tenant in every durable lookup and make cross-tenant absence indistinguishable from missing; resolution status derives tenant from verified SPIFFE identity and returns one generic not-found response | Query introduced without tenant predicate; timing differences still require deployment measurement |
