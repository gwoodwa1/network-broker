...
| Prompt injection or poisoned device text | Preserve hostile captured bytes separately; apply versioned bounded sanitisation, Unicode-normalized detection, safe quarantine, strict schema promotion and signed field-level taint | Novel obfuscation, false negatives in configured rules or an upstream consumer that treats tainted data as instructions |
