...
- Resumable, tenant-scoped resolution event streams using safe version cursors.
...
PostgreSQL deployments must apply the versioned scripts in `migrations/` before constructing the repository with an application-owned `*sql.DB`. Resolution creation canonicalises the bounded v1 request, computes its digest server-side and atomically stores the exact request bytes, digest, actor-and-tenant-scoped idempotency record and creation event. Reusing an idempotency key for the same canonical request returns the existing workflow; reusing it for different content fails closed. See the [resolution creation API](docs/resolution-create-api.md) and [resolution idempotency contract](docs/idempotency.md).
...
`collectorruntime.New` constructs the collection pipeline without a memory-store fallback: task authority, artefact metadata, evidence envelopes and grant consumption use PostgreSQL, while immutable blob storage, signing, encryption, transport, policy and credential exchange remain mandatory deployment-supplied boundaries. Authenticated agents create durable workflows through `POST /v1/resolutions`; the server canonicalises and persists their bounded claim/target request and digest. `CreateFanoutContext` atomically queues a planned resolution, creates its complete task set and appends the outbox event. The authenticated `POST /v1/resolutions/{resolution_id}/tasks:queue` planning endpoint derives tenant authority from a verified planner SVID, strictly decodes a bounded v1 plan and supplies server-owned task state, event identity and event time. Agents read durable workflow state through tenant-scoped `GET /v1/resolutions/{resolution_id}` or watch safe resumable SSE events at `GET /v1/resolutions/{resolution_id}/events`. Watch cursors use per-resolution versions rather than global outbox sequences, and raw event payloads never cross the API. The control plane continuously runs bounded expired-evidence reconciliation and exports candidate, accepted, skipped and failure counters. See the [resolution creation API](docs/resolution-create-api.md), [resolution status API](docs/resolution-status-api.md), [resolution watch API](docs/resolution-watch-api.md) and [production authority runtime](docs/production-authority-runtime.md).

The control-plane entrypoint requires `DATABASE_URL`, the exact non-administrative `DATABASE_ROLE`, `NATS_URL`, and a deployment-unique `OUTBOX_WORKER_ID`. It verifies the live PostgreSQL role, audits its effective table and sequence grants against an exact manifest, and verifies NATS connectivity before becoming ready. It exposes `GET /livez`, `GET /readyz`, and Prometheus-format `GET /metrics` endpoints. If `APPLY_MIGRATIONS=true`, a distinct `MIGRATION_DATABASE_URL` is required; that identity is closed before the runtime database connection is opened. Concurrent migration attempts are serialised with a PostgreSQL advisory lock. See [PostgreSQL runtime-role enforcement](docs/database-role-enforcement.md). `LISTEN_ADDRESS` defaults to `:8080`.

Expired-evidence reconciliation is mandatory in the control-plane runtime. `RECONCILIATION_BATCH_SIZE` defaults to `100`, `RECONCILIATION_POLL_INTERVAL` to `5s`, and `RECONCILIATION_FAILURE_DELAY` to `5s`. Invalid, zero or excessive bounds fail startup. Multiple replicas may run the scheduler because acceptance rechecks every authority binding atomically.
...
Read-only gNMI, NETCONF and SSH adapters enforce exact catalogue recipe versions, short-lived opaque credential exchange, deadlines and byte bounds. gNMI requires TLS 1.3 with explicit roots and hostname verification; NETCONF and SSH require verified SSH host keys. The first gNMI interface-state normaliser preserves raw protobuf JSON, applies hostile-output inspection and accepts only a bounded OpenConfig `name`/`oper-status` response before producing the strict parser schema. See the [gNMI interface-state profile](docs/gnmi-interface-state-profile.md), [transport security](docs/transport-security.md), the [threat model](docs/threat-model.md) and [production hardening](docs/production-hardening.md).
...
- A deployment entrypoint supplying qualified transport, policy, credential-broker, S3, KMS and verified workload-identity dependencies to the durable collector runtime.
- QueryContext and evidence-retrieval network APIs; resolution creation, status and resumable watch are now implemented.
...
- Production runtime wiring for disclosure services; formal algorithm lifecycle policy; and experimental standardized post-quantum providers.
